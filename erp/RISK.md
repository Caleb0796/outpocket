...
- **The `confirm_token` is defence in depth, not a proof.** `S5` (+0.5 h, now 4.0)
  ...
  not establish personhood.** Its value is a direct function of **open unknown
  ...
  caller. State the residual risk in exactly those terms wherever the gate is
  described — §4's `V3` row is the same unknown, and it got **more** load-bearing
  under R-1, not less.
