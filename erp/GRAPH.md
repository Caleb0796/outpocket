...
>
> Regenerated 2026-08-29 from `graph.json` (68 nodes, 124 edges — 113 hard and
...
> - The five unknowns are keyed **`V0`–`V4`** — **all five MEASURED 2026-08-29 (R-43)**, plus a
>   discovered sixth keyed **`V6-consent-gate`** — and are answered by the V-lane nodes of the
...
Applied: all 124 edges carry a `contract` naming the file that crosses them.
...
**Two more were missing and v2.1.0 drew them; one of the two is since DELETED.** `G1 → G3` was
drawn on the belief that G3's clone runs over *anonymous* HTTPS and would fail authentication
against a private repo. **R-42/D-30 deleted it, 2026-08-29, MEASURED**: `git clone` of a PRIVATE
repo succeeds when the CLI is authenticated (52 files cloned from `Caleb0796/outpocket` while
private). G3 needs the PUSH (L0 gate 6), not the visibility flip. `G3.inputs` is `[T6, L0]` and
`G1` moves to Day 6. This deletion is the whole of the 121→120 / 110→109 / 102→101 edge-count
correction recorded in `conventions.cut_invariant_grade`.
...
| V6 | Unknowns verdict and fallback election | PM | V0 (soft), V1, V2 (soft), V3 (soft), V4 (soft) | 1.5 | 0 | `tools/check-unknowns.mjs` exits 0: `evidence/UNKNOWNS.md` has **exactly six** rows — the five original unknowns **V0, V1, V2, V3, V4** plus the discovered sixth **`V6-consent-gate`** (R-43) — each ending in the literal `MEASURED` or `UNVERIFIED`, and every `UNVERIFIED` row names an existing node id from `graph.json` as its fallback. **The sixth key carries its suffix and is never a bare `V6`**, which is this node's own id; a row pattern matching bare `V6` matches a node id. The count is asserted, not floored |
...
| S5 | Human sign gate: server-owned decision state, snapshot-digest binding, one-shot guard | I3 | S4, S11 | 4 | 0 | **Shipped mode is the two-call handshake, not suspend-until-signed — `contingencies[4]` fired (R-43): V4 is MEASURED at 22.3 s and nobody signs a report in 22 seconds.** `drive.mjs --scenario sign`, in mode `handshake`: execute **resolves** inside 2 s with `{status:'awaiting_signature', ticket}` carrying **no** `confirm_token`, **no** `snapshot_digest` and **no** revision (asserted by key absence); a record in state `open`; snapshot digest == `digest('outpocket/snapshot/1', snapshot)` with **`request_id` inside the projection**; **the sign lock held across the gap between the two calls**, asserted by a mutating request returning 423 `E_SIGN_IN_PROGRESS` — the freeze was always the lock, never the unresolved promise, which is why the mode change costs no guarantee; and the record reaching `answered` only after `POST /api/sign/{request_id}/respond`, observed from a second tool call presenting the ticket. **The ticket is not a credential**: a respond body presenting it in place of the `confirm_token` is 403. **The handshake is not a mitigation of the surviving forgery** — withholding the digest from the tool *result* does not withhold it from a caller who can `GET /api/sign/{request_id}`, so the walk costs one extra request and is otherwise unchanged. AND mode `suspend`, driven against `harness/fake-client` (no timeout), still asserts the old shape — that arm makes the switch real and is **expected unrunnable** against the built-in browser. AND `tests/signature.test.mjs` reproduces every published digest. AND `tests/acceptance/sign-state.test.mjs` asserts **N-15** (a synthesised client-side sign response → 409 `E_NOT_SIGNED`), **N-16 `neg-respond-without-click`** scripted verbatim as the attack and recorded honestly — **it COMMITS today** (controlStatus `known-open`) and returns 403 `E_NO_CONFIRM_TOKEN` (controlStatus `enforced`) only for a caller that cannot read the dialog's DOM, **and `V3` is MEASURED `same-session`, so that caller is not the one this demo runs in** — and the **`confirm_token`**, minted with the sign request, delivered **only** into the rendered dialog's DOM and never into any tool-call result. AND the **scheduled deviation `DEV-E3-eval-case-known-open`**: the `confirm_token` flips `erp/contracts/eval-case.schema.json` `examples[1]` from `known-open`/"IT COMMITS" to **`enforced`** (R-27: `refused` is **not** in the frozen enum `[enforced, known-open, not-runnable]`, and the gate that demanded it would have failed ajv and turned `npm test` red repo-wide on Day 3), that file was frozen by S10 on Day 1 under `sha256sum -c`, and E3 (Day 4, downstream through S6) failed by construction until this node filed the ticket, landed the edit in the same commit and re-recorded the digest in `erp/contracts/FREEZE.md` |
...
| F6 | Demo skin aligned to the storyboard shot ids | UX | F0, F2 (soft), F4 (soft), F5 (soft), F7 | 2.5 | 1 | `tools/check-storyboard.mjs` exits 0: every shot id in `docs/STORYBOARD.md` resolves to a CSS selector matching ≥1 element on the built page |
...
| D4 | Video under 3 minutes, with audio, English, mechanism in the first 10 seconds | UX | H6, D1, F0, F7 | 4 | 0 | `ffprobe` reports duration <180 AND ≥1 stream with `codec_type=audio`; the public URL returns 200 from a logged-out fetch; and `docs/VIDEO-SCRIPT.md`'s first cue is ≤00:10 and contains a literal token from `kb/webmcp/MECHANISMS.txt` |
...
- "the surface changes **on the spot**" → MEASURED (**V2 = `refreshes`**, 2026-08-29): the tool
  list *does* refresh with no reload, and the honest phrasing is still "on its next turn",
  because what refreshed is the list the agent reads on its next turn. The storyboard
  re-prompts the agent after the flip
