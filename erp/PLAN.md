...
it does **not** establish personhood; and its value is a direct function of **open
unknown V3**. If an agent-initiated fetch carries the page session cookie and the
agent can read the DOM, the token is reachable and N-16 stays `KNOWN-OPEN` for that
caller (`graph.json.contingencies`, owner L2).
...
`execute` suspends; a visible dialog shows the worst-case consequence above the
...
re-prompt is open unknown **V2**; until V2 returns `refreshes`, the storyboard
re-prompts the agent after the flip and the narration says "on its next turn",
never "on the spot". The absence register (**T3**) then tells it, in structured
...
|---|---|---|---|
| **2.5 h/day × 5.5 days** | 13.75 | 15.875 | Does **not** fit, short by 2.125 |
| **3.0 h/day × 5.5 days** | 16.5 | 15.875 | Fits. **Nothing needs cutting**, 0.625 h spare |

At 2.5 h/day the ladder must be fired to **rank 3** — the first rank that fits, at
...
| D-17 | **The human daily budget is 2.5 h/day (13.75 h total) unless PM rules otherwise on Day 0, before any seat is dispatched.** At 2.5 the full graph does not fit and ranks 1–3 fire, deleting the 27 nodes named in §6.4. At 3.0 nothing is cut. **PM records the ruling as a `human_hours_per_day = 2.5\|3.0` row in `erp/DECISIONS.md`, which `L0` CREATES on Day 0 for this purpose**; `L0` accept **gate (1)** is the gate, and it fails unless the row parses and `graph.json`'s `capacity.human_hours_available` equals the ruled figure × 5.5. `V6` later appends its unknowns rows to the same file | The whole ladder hangs on half an hour per day. The previous revision reported the deficit with no node and no gate, then gave the ruling a home in `erp/DECISIONS.md` — **a `V6` output on Day 2**, while Day 0 held only `L0` and `V5`, so a ruling due "before any seat is dispatched" was due into a file that did not yet exist. Ruling R-10; home and gate closed this revision | OUR-ESTIMATE on the required side (15.875 h); PUBLISHED on the available side (the user's own stated 2–3 h/day) |
...
| **V4 reports a timeout** | A suspended `execute` times out before the sign gate can complete | "S5 must ship the two-call handshake mode, not the suspend-until-signed mode. S5 is written with BOTH modes behind one switch from the start; this is not a Day-4 rewrite." Owner: I3 |
...
| **The human budget turns out to be 2.5 h/day** | D-17 settles at 2.5 | The full graph does not fit — 13.75 available against 15.875 required. Ranks 1–3 fire and 27 of 62 horizon-A nodes are deleted, leaving 0.275 h of spare. At 3.0 h/day nothing is cut and 0.625 h is spare. **And note what the ladder cannot touch:** Day 5 and Day 6 each carry 4.0 human-gated hours against a 2.5 h/day average, all of it cut 0. If that is the problem, cut D4's scope — §6.4 |
