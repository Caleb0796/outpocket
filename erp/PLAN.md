...
### Claim (b) — the site can attest to co-presence, and turns presence into auditable evidence

**Precisely:** at the moment a consequential write happens, outpocket can show —
and later prove from its own records — that **a specific authenticated human**
reviewed **a specific canonical snapshot**, that **the write arrived through the
tool surface** rather than the UI, and that what was persisted is byte-identical
to what the human saw.
...
it does **not** establish personhood; and its value was a direct function of
unknown **V3**, which is now **MEASURED `same-session`** — the unfavourable answer.
An agent-initiated fetch in the ChatGPT built-in browser **does** carry the page
session cookie (2026-08-29, `evidence/V3.json`) and that client reads the DOM, so
the token **is** reachable and N-16 **stays `KNOWN-OPEN` for that caller**. Present
tense, not conditional (`graph.json.contingencies[3]`, **fired**; R-43; owner L2).
One thing raises the attacker's cost and it is **not ours**: on a remote origin that
same client gates the cookie-bearing POST behind an action-time human approval
(`V6-consent-gate`). That is a client policy, absent on localhost, defeated by a
different client or a reflexive click — **cost, never closure**, and no document may
cite it without that clause.
...
> `docs/VIDEO-SCRIPT.md` gives it its own cue and the narrator names it: *the
> client is asking the human to be present for this call — which is the thing the
> page is arguing for.* Two hard rules on that line. It is **not** evidence that
> our design works: it is a **client policy**, it is absent on localhost, and no
> claim anywhere may rest on it (R-43). And the rehearsal must be done on the
> **remote** origin — a localhost rehearsal will not show the prompt and will
> leave the crew surprised on the take that counts.
...
> **V6** — `node tools/check-unknowns.mjs` exits 0 — it requires evidence/UNKNOWNS.md to contain exactly 5 rows keyed V0, V1, V2, V3, V4, each ending in the literal token MEASURED or UNVERIFIED, and requires every UNVERIFIED row to name an existing node id from erp/graph.json as its fallback.
...
| Node | Owner | h | Cut | Title |
|---|---|---|---|---|
| **S5** | I3 | 4 | 0 | Human sign gate: server-owned decision state, snapshot-digest binding |
