...
| **V2** | Does the tool list refresh mid-session, with no reload? | **refreshes** (localhost) | A sixth tool registered at runtime reached the agent with no page reload, on a **subsequent turn** of the same conversation. It does **not** establish that a surface change reaches the model mid-turn. Narration stays "on its next turn"; the instant-update phrasing registered as BW-33 remains forbidden. Not re-run remotely. MEASURED
